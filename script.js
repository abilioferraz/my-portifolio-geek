...
function createActionButton(item) {
  const action = createElement("a", `button button--${item.variant}`, item.label);
...
function createTagList(tags) {
  ...
  tags.forEach((tag) => {
    list.append(createElement("li", "", tag));
  });
...
function renderTimeline() {
  ...
  portfolioConfig.timeline.forEach((item) => {
    const article = createElement("article", "timeline__item");
    article.setAttribute("data-reveal", "");

    const meta = createElement("span", "timeline__meta", item.meta);
...
function renderStack() {
  ...
  portfolioConfig.stack.forEach((item) => {
    const article = createElement("article", "stack-card");
    article.setAttribute("data-reveal", "");

    const level = createElement("span", "stack-card__level", item.level);
    const title = createElement("h3", "", item.title);
    const description = createElement("p", "", item.description);

    article.append(level, title, description);
    fragment.append(article);
  });
...
function renderQuestLog() {
  const container = document.querySelector("#quest-grid");
  ...
  portfolioConfig.quests.forEach((item) => {
    const article = createElement("article", "quest-card");
    article.setAttribute("data-reveal", "");

    const tag = createElement("span", "quest-card__tag", item.tag);
    const title = createElement("h3", "", item.title);
    const description = createElement("p", "", item.description);
...
function renderValueProps() {
  ...
  portfolioConfig.valueProps.forEach((item) => {
    const article = createElement("article", "value-card");
    article.setAttribute("data-reveal", "");

    const tag = createElement("span", "value-card__tag", item.tag);
    const title = createElement("h3", "", item.title);
    const description = createElement("p", "", item.description);
...
function renderContacts() {
  ...
  if (ctaActions) {
    const fragment = document.createDocumentFragment();
    portfolioConfig.actions.cta.forEach((action) => fragment.append(createActionButton(action)));
    ctaActions.replaceChildren(fragment);
  }
}
